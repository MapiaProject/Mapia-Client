...
void CustomVisualizer::DrawVisualization(const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	if (mapLayoutViewer)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("path : %s"), *FPaths::ProjectUserDir());
		FString path = FPaths::ProjectUserDir() + TEXT("Source\\WOS\\Network\\generated\\mapData\\") + mapLayoutViewer->mapName + TEXT(".ini");
		//파일이 없을경우 기즈모 숨기기
